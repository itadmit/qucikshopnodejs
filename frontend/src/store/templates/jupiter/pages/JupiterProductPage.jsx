...
const JupiterProductPage = ({ storeData }) => {
  ...
  const fetchPageDesign = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://api.my-quickshop.com/api'}/stores/${storeData.slug}/design/product-page`)
      if (response.ok) {
  ...
  const fetchProductData = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://api.my-quickshop.com/api'}/stores/${storeData.slug}/products/${slug}`)
            ...
            <div className="aspect-square bg-gray-100 rounded-lg overflow-hidden">
              {product.images && product.images.length > 0 ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-gray-50">
                  <div className="text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-24 h-24 text-gray-400 mb-4" aria-hidden="true">
