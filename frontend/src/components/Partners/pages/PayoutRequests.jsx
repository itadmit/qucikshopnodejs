...
const PayoutRequests = ({ partner }) => {
  ...
  const fetchPayouts = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/partners/payouts?${params}`, {
        headers: {
  ...
  const fetchAvailableBalance = async () => {
    try {
      const token = localStorage.getItem('partnerToken');
      const response = await fetch('http://localhost:3001/api/partners/commissions/balance', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
...
const PayoutRequestModal = ({ availableBalance, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('partnerToken');
      const response = await fetch('http://localhost:3001/api/partners/payouts/request', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
